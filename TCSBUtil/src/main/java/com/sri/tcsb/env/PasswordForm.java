...
public class PasswordForm extends JFrame implements ActionListener {
	...
	TCSBLogger log = TCSBLogger.getLogger(PasswordForm.class.getName());
	...
	public PasswordForm() {
        ...
        try{
        	Properties pwds = TCSBPassword.getAllPasswords();
        	if (pwds!=null){
                if (pwds.containsKey(TCSBEnvProp.DBPwd)){
                	dbPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(TCSBEnvProp.DBPwd)));
                } 
                if (pwds.containsKey(TCSBEnvProp.EmailPwd)){
                	emailPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(TCSBEnvProp.EmailPwd)));
                } 
                if (pwds.containsKey(TCSBEnvProp.SmsPwd)){
                	smsPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(TCSBEnvProp.SmsPwd )));
                } 
        	}
        }catch(Exception er){
	...
	public void actionPerformed(ActionEvent actionEvent) {
		if (actionEvent.getActionCommand().equals("Save")){
			...
			try{
				pwds.put( TCSBEnvProp.DBPwd, CipherTextInfo.encrypt(String.valueOf(dbPwd_txt.getPassword())));
				pwds.put( TCSBEnvProp.EmailPwd, CipherTextInfo.encrypt(String.valueOf(emailPwd_txt.getPassword())));
				pwds.put( TCSBEnvProp.SmsPwd, CipherTextInfo.encrypt(String.valueOf(smsPwd_txt.getPassword())));
				TCSBPassword.savePasswords(pwds);
			}catch(Exception er){
